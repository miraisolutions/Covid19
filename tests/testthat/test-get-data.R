...
test_that("get_daily_data returns expected headers", {
  ...
  expect_equal(names(data), c("Province.State", "Country.Region", "Last.Update", "Confirmed", "Deaths", "Recovered" ))
})

data_full <- get_timeseries_full_data()

test_that("get_timeseries_full_data returns expected headers", {
  expect_equal(names(data_full), c("Province.State", "Country.Region", "Lat", "Long", "date", "confirmed", "deaths", "recovered", "active"))
...
test_that("get_timeseries_by_contagion_day_data returns expected headers", {
  expect_equal(names(data), c("Province.State", "Country.Region", "Lat", "Long", "date", "confirmed", "deaths", "recovered", "active", "contagion_day"))
...
test_that("get_timeseries_global_data returns expected headers", {
  ...
  expect_equal(names(df), c("date", "confirmed", "deaths", "recovered", "active"))
...
test_that("get_timeseries_country_data returns expected headers", {
  ...
  expect_equal(names(df), c("date", "confirmed", "deaths", "recovered", "active", "contagion_day"))
  expect_false(any(df$active < 0))
})
...
test_that("aggregate_province_timeseries_data returns expected headers", {
  ...
  expect_equal(names(df), c("Country.Region", "date", "confirmed", "deaths", "recovered", "active", "contagion_day"))
})
